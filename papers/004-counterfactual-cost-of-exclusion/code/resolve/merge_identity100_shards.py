...
concatenated deterministically by shard directory name. Duplicate Author/Work
records are collapsed by OpenAlex ID while resolution-audit rows remain
candidate-specific.
"""
...
def id_key(row: dict[str, Any]) -> str:
    value = str(row.get("id") or row.get("openalex_id") or "").strip()
...
def main() -> int:
    ...
    for directory in dirs:
        ...
        for row in read_jsonl(directory / "openalex_authors.jsonl"):
            key = id_key(row)
            if key:
                authors_by_id.setdefault(key, row)
        for row in read_jsonl(directory / "openalex_works.jsonl"):
            key = id_key(row)
            if key:
                works_by_id.setdefault(key, row)
    ...
    with (args.out_dir / "openalex_authors.jsonl").open("w", encoding="utf-8") as handle:
        for key in sorted(authors_by_id):
            handle.write(json.dumps(authors_by_id[key], ensure_ascii=False) + "\n")
    with (args.out_dir / "openalex_works.jsonl").open("w", encoding="utf-8") as handle:
        for key in sorted(works_by_id):
            handle.write(json.dumps(works_by_id[key], ensure_ascii=False) + "\n")

    summary = {
        "expected_shards": args.expected_shards,
        "shards_found": len(dirs),
        "candidate_audit_rows_n": len(audit),
        "unique_author_records_n": len(authors_by_id),
        "unique_work_records_n": len(works_by_id),
